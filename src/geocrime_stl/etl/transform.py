...
def clean_and_spatial_clip(
    ...
    # Structural Re-mapping & Column Dropping
    ...
    # Regex Character Stripping (e.g., "G-88" -> 88)
    ...
    # Apply Final Schema Alignment from global configuration
    existing_cols = [col for col in FINAL_SCHEMA_COLUMNS if col in df.columns]
    df = df[existing_cols]

    # Spatial Boundary Clipping Execution
    try:
...
def run_pipeline(month: int, year: int) -> CrimeDataPackage:
    ...
    # Fetch the data stream from SLMPD with built-in safety net
    try:
    ...
    # Clean, type-enforce, and clip to city boundaries
